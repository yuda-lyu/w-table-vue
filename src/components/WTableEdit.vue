    ...
    >
        <template v-if="hasEffUseOpt">
            ...
            >
                ...
                <!-- 轉發cell-render, cell-tooltip, head-render, head-tooltip至w-aggrid-vue, 須加v-if守衛: w-aggrid-vue偵測到slot存在即改用slot renderer, 若無條件轉發, 呼叫端未給slot時儲存格與head會被渲染成空白 -->
        ...
        downloadData: function(mode = 'raw') {
            ...
            try {
                ...
                fun({
                    ...
                    useFormat: vo.useCellFormatWhenDownload, //default: true, 依opt.kpCellFormat格式化使下載與畫面一致
